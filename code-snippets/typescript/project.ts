...
type U = 'A' | 'B' | 'C'
type t11 = `${Exclude<U, 'B' | 'C'>}${Exclude<U, 'A' | 'C'>}${Exclude<
  U,
  'A' | 'B'
>}`
...
type Chainable1<T = {}> = {
  option<K extends string, V extends unknown>(
    ...
    v: V
  ): Chainable1<(K extends keyof T ? Omit<T, K> : T) & { [P in K]: V }>
  get(): T
}
...
type TAddPropType = Omit<AddPropType<Customer, 'weixin', string>, never>

type Heavy<T> = `${Uppercase<T & string>}`

type BEM<
  T extends string,
  U extends Array<string>,
  M extends Array<string>
> = `${T}__${U[number]}--${M[number]}`

type bemResult = BEM<'guang', ['aaa', 'bbb'], ['warning', 'success']>

type len = [1, 2, 3]
type len2 = number[]
type IsTuple<A extends unknown[]> = A['length'] extends number
  ? number extends A['length']
    ? false
    : true
  : false

/* type TIsTuple = IsTuple<len>
type TIsTuple1 = IsTuple<len2> */
...
type IsTupleByEquel<T> = T extends [...infer Eles]
  ? NotEqual<Eles['length'], number>
  : false

type TIsTuple = IsTupleByEquel<len>
type TIsTuple1 = IsTupleByEquel<len2>
...
type UnionAndInsert = { guang: 1; dong: 2 } extends { guang: 1 } | { dong: 2 }
  ? 1
  : 2
...
interface AppConfig {
  ...
  entry: string
  container: `#${string}`
  activeRule: `/${AppConfig['name']}`
}

function setDefaultMountApp<T extends readonly unknown[]>(name: AppName<T>) {}
...
type AppName<T extends readonly unknown[]> = T extends readonly [
  infer F extends AppConfig,
  ...infer R extends AppConfig[]
]
  ? F['name'] | AppName<R>
  : never

type TA = (string | number)[] extends unknown ? 1 : 2

setDefaultMountApp<typeof configs>('restaurant')
