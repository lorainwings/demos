...
type IPReadonly<T extends Record<keyof any, any>> = any extends any
  ? {
...
type TCondition<Positive, Condition, V1, V2> = Positive extends Condition
  ? V1
  : V2
...
type tss1 = { vipExpires?: number } & (
  | { vipExpires?: undefined; promotionUsed: boolean }
  | { promotionUsed?: undefined; refererType: string }
)
...
type ThreeMutex =
  | (Without<VIP, TCommonUserInfo> & TCommonUserInfo)
  | (Without<TCommonUserInfo, VIP> & VIP)
...
type XORUserSpread =
  | (Without<VIP, XORCommonUserInfo> & TCommonUser)
  | (Without<VIP, XORCommonUserInfo> & TVisitor)
  | (Without<XORCommonUserInfo, VIP> & VIP)
type XORUserSpread1 =
...
// 方式二
type MutexVipUserInfo = Flatten<
  Without<CommonUserInfo, CommonUserInfo | VisitorUserInfo> &
    Without<VisitorUserInfo, CommonUserInfo | VisitorUserInfo> &
    VipUserInfo
>
type MutexCommonUserInfo = Flatten<
  Without<VipUserInfo, CommonUserInfo | VisitorUserInfo> &
    Without<VisitorUserInfo, CommonUserInfo | VisitorUserInfo> &
    CommonUserInfo
>
type MutexVisitorUserInfo = Flatten<
  Without<CommonUserInfo, CommonUserInfo | VisitorUserInfo> &
    Without<VipUserInfo, CommonUserInfo | VisitorUserInfo> &
    VisitorUserInfo
>
type XORStructUserInfo =
  | MutexVipUserInfo
  | MutexCommonUserInfo
  | MutexVisitorUserInfo
...
type uaaa = keyof (
  | { v: number; c?: boolean; r?: string; b: object }
  | { v: number; c?: boolean; r?: string; de: object }
)
...
type IsEquel<T, U> = (<X>() => X extends T ? 1 : 2) extends <X>() => X extends U
  ? 1
  : 2
  ? true
  : false
...
type StartWith<
  T extends string,
  S extends string
> = T extends `${S}${infer Rest}` ? true : false
...
> = List extends [
  ...
  ...infer Rest extends Array<number | string>
]
...
type SnakeCase2CamelCase<S extends string> = S extends `${infer L}_${infer R}`
