...
function processVueFile(content) {
  ...
  while ((styleMatch = styleRegex.exec(content)) !== null) {
    ...
    while ((ruleMatch = ruleRegex.exec(styleContent)) !== null) {
      ...
      // 跳过 @mixin text-base 相关规则
      if (selector.includes('@mixin') || selector.includes('text-base')) {
        continue
      }

      let modifiedDeclarations = declarations

      // 处理每个声明
      CONFIG.targetProps.forEach((prop) => {
        const propRegex = new RegExp(`(${prop.replace('-', '\\-')}\\s*:\\s*)(\\d+(\\.\\d+)?px)`, 'g')
        let propMatch
        ...
        while ((propMatch = propRegex.exec(declarations)) !== null) {
          ...
          if (isPxValue(pxValue)) {
            const newValue = multiplyPxValue(pxValue, CONFIG.multiplier)

            if (pxValue !== newValue) {
              const oldFullMatch = fullMatch
              const newFullMatch = prefix + newValue

              modifiedDeclarations = modifiedDeclarations.replace(oldFullMatch, newFullMatch)
      ...
      // 更新样式内容
      if (modifiedDeclarations !== declarations) {
        const oldRule = ruleMatch[0]
        const newRule = ruleMatch[1] + '{' + modifiedDeclarations + '}'
        modifiedStyleContent = modifiedStyleContent.replace(oldRule, newRule)
      }
    }

    // 更新整个 style 标签
    if (modifiedStyleContent !== styleContent) {
      const oldStyleTag = styleMatch[0]
      const newStyleTag = styleMatch[0].replace(styleContent, modifiedStyleContent)
      modifiedContent = modifiedContent.replace(oldStyleTag, newStyleTag)
    }
  }

  return { content: modifiedContent, adjustments }
}

// 处理 SCSS/CSS 文件
function processStyleFile(content, filePath) {
  const adjustments = []
  let modifiedContent = content

  // 跳过 @mixin text-base 相关规则
  if (content.includes('@mixin text-base')) {
    log.info(`跳过包含 @mixin text-base 的文件: ${filePath}`)
    return { content: modifiedContent, adjustments }
  }
...
function processFile(filePath) {
  try {
    ...
    if (ext === '.vue') {
      result = processVueFile(content)
    } else if (ext === '.scss' || ext === '.css') {
      result = processStyleFile(content, filePath)
    } else {
      return {
        filePath,
        adjustments: [],
    ...
    results.forEach((result) => {
      if (result.adjustments.length > 0) {
        ...
        result.adjustments.forEach((adj) => {
          report += `- ${adj.property}: ${adj.oldValue} → ${adj.newValue} (选择器: ${adj.selector})\n`
        })
        report += `\n`
      }
    })
...
export { processFile, processVueFile, processStyleFile, multiplyPxValue }
