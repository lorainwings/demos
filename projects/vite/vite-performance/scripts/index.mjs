...
process.stdin.on('data', async (chunk) => {
  ...
  await fs.readFile(path.resolve('./src/pages', 'multiPages.json'), 'utf-8', (err, data) => {
    //获取老数据
  ...
  function setMapFile(datas) {
    // 通过writeFile改变数据内容
    fs.writeFile(path.resolve('./src/pages', 'multiPages.json'), JSON.stringify(datas), 'utf-8', (err) => {
      if (err) throw err
      // 在pages中建立新的目录
